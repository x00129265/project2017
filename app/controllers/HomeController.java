...
//JT----Product ands categories------------------------------------
       List<Category> categoriesList = Category.findAll();
        List<Product> productsList = new ArrayList<Product>();
        List<Product> newProductList = new ArrayList<Product>();
        if (cat == 0) {
            // Get list of all categories in ascending order
            productsList = Product.findAllNotZero(filter);
        }
        else {
...
//JT----Slider---------------------------------------------------
       ...
       return ok(index.render(env, categoriesList, newProductList, getUserFromSession(), cat, filter, iList, activeItem));
   }
