...
public class Product extends Model {
    ...
    // List of ids - this will be bound to checkboxes in the view form
...
private Plike lds2;
    ...
    public static List<Product> findAllNotZero(String filter) {
        return Product.find.where()
                .ilike("name", "%" + filter + "%")
                .ne("stock", 0)
                .orderBy("name asc")
    ...
    public static List<Product> findFilterNoZero(Long catID, String filter) {
        return Product.find.where()
                        // Only include products from the matching cat ID
                        // In this case search the ManyToMany relation
                        .eq("categories.id", catID)
